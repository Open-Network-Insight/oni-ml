...
object DnsPreLDA {
  ...
  val parser = new scopt.OptionParser[Config]("DNSPreLDA") {
  ...
  def run(args: Array[String]) = {
    parser.parse(args.drop(1), Config()) match {
      ...
      case Some(config) => {
        ...
        val feedbackFile = config.scoresFile
        ...
        val falsePositives: org.apache.spark.rdd.RDD[String] = if (scoredFileExists) {
          ...
          */
          ...
          val toDuplicate: org.apache.spark.rdd.RDD[String] = result.flatMap(x => List.fill(config.duplicationFactor)(x))
          toDuplicate
        } else {
          null
        }
