...
class DNSSuspiciousConnectsModel(inTopicCount: Int,
  ...
  /**
    ...
    * @return Dataframe with a column named [[org.opennetworkinsight.dns.DNSSchema.Score]] that contains the
...
object DNSSuspiciousConnectsModel {
  ...
  def trainNewModel(sparkContext: SparkContext,
    ...
    val OniLDACOutput(ipToTopicMix, wordToPerTopicProb) = OniLDACWrapper.runLDA(ipDstWordCounts,
      config.modelFile,
